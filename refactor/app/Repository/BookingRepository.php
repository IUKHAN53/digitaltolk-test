...
class BookingRepository extends BaseRepository
{
    ...
    function __construct(Job $job, MailerInterface $mailer)
    {
        ...
        $log_path = storage_path('logs/admin/laravel-' . date('Y-m-d') . '.log');
        $this->logger->pushHandler(new StreamHandler(storage_path($log_path), Logger::DEBUG));
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    public function jobEnd(array $post_data = array())
    {
        ...
        $this->sendUseEmail($tr, $job, $session_time, $completeddate);
    ...
    public function sendNotificationTranslator($job, $exclude_user_id, array $data = [])
    {
        ...
        foreach ($users as $oneUser) {
            if ($oneUser->user_type == '2' && $oneUser->status == '1' && $oneUser->id != $exclude_user_id) { // user is translator and he is not disabled
                if (!$this->sendingPushNeeded($oneUser->id)) continue;
                    ...
                    if ($job->id == $oneJob->id) { // one potential job is the same with current job
                        ...
                        if ($job_for_translator == 'SpecificJob') {
                            ...
                            if (($job_checker != 'userCanNotAcceptJob')) {
                                if ($this->pushDelayNeeded($oneUser->id)) {
     ...
     * @return bool
     */
    public function pushDelayNeeded($user_id): bool
    {
     ...
     * @return bool
     */
    public function sendingPushNeeded($user_id): bool
    {
...
//        return false;
    ...
    public function sendExpiredNotification($job, $user)
    {
        ...
        if ($this->sendingPushNeeded($user->id)) {
            $users_array = array($user);
            $this->sendPushNotificationToSpecificUsers($users_array, $job->id, $data, $msg_text, $this->pushDelayNeeded($user->id));
        }
    }
        ...
        if ($currentUser->is('customer')) {
            ...
            if ($translator) {
                ...
                if ($this->sendingPushNeeded($translator->id)) {
                    ...
                    $this->sendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text, $this->pushDelayNeeded($translator->id));// send Session Cancel Push to Translaotor
                }
            }
        } else {
            if ($job->due->diffInHours(Carbon::now()) > 24) {
                ...
                if ($customer) {
                    ...
                    if ($this->sendingPushNeeded($customer->id)) {
                        ...
                        $this->sendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text, $this->pushDelayNeeded($customer->id));     // send Session Cancel Push to customer
                    }
                }
                $job->status = 'pending';
                $job->created_at = date('Y-m-d H:i:s');
